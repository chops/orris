...
:END:
To inspect journals under an explicit local runs root:

#+begin_src bash
ai-orchestrator list --root /path/to/runs --json
#+end_src

This reports verified journal status and pending repair without changing the runs.
See [[file:docs/contracts/cli-discovery.org][explicit-root discovery]] for output and scope.
...
Prompt delivery requires Bash available as ~bash~ on ~PATH~; the pinned Nix
development shell provides it.
...
Implementation language policy: [[file:docs/language-policy.org][Elixir and Rust]].
