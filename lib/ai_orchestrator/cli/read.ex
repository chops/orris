defmodule AiOrchestrator.CLI.Read do
  ...
  Every directory-based operator read (`status` and the legacy `list` rows) folds the verified journal prefix
  through `AiOrchestrator.Prepare.Trusted.read_journal/1` and renders the same value. The loaded map's
